...
@app.get("/healthz")
async def healthz():
    return {"status": "ok", "service": "vela-adapter", "calls_processed": len(call_log)}
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card-accent {
    width: 4px; border-radius: 2px; align-self: stretch; min-height: 60px;
  }
  ...
  .badge-urgent    { background: #f8514920; color: #f85149; border: 1px solid #f8514940; }
...
<div class="queue">
  ...
  <div id="cards"><div class="empty"><div class="empty-icon">&#9743;</div><div class="empty-text">No calls yet. POST a transcript to /intake/voice to see AI analysis here.</div></div></div>
</div>

<footer>vela-adapter &bull; router:gmi-service-desk &bull; <a href="/calls" style="color:#58a6ff">JSON API</a> &bull; <a href="/healthz" style="color:#58a6ff">Health</a></footer>
...
async function refresh() {
  ...
  if (!calls.length) {
    document.getElementById('cards').innerHTML = '<div class="empty"><div class="empty-icon">&#9743;</div><div class="empty-text">No calls yet. POST a transcript to /intake/voice to see AI analysis here.</div></div>';
    return;
  }
  ...
  const html = calls.map((c, idx) => {
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="card-top">
          <span class="caller">${escHtml(c.caller_id || 'Unknown')}</span>
          ${badges.join('')}
